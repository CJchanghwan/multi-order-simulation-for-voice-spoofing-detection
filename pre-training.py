...
parser.add_argument('--save_path', type=str, default='/voice_spoofing_detection/acoustic_resnet.pt', help='path to save')
...
train_dataset = dataset.Multiorder(vctk_path, vctk_label ,augment,train_room)
train_loader = DataLoader(train_dataset, batch_size=64,  num_workers=40,shuffle = True,drop_last = True)

test_dataset = dataset.Multiorder(vctk_path, vctk_label,augment,val_room)
...
criterion = utils.LossFunction(256,3).to(device)
...
def train(epoch):
    ...
    for epoch in range(epoch):
        ...
        with torch.no_grad():
            ...
            if val_eer > best :
                torch.save(model.state_dict(),config.save_path) # non_aug #masking # speed
                best = val_eer
                best_epoch = epoch
                print("model saved")
